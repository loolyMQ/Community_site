...
export default function Orb({
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.warn('WebGL не поддерживается, орб не будет отображаться');
      return;
    }
    ...
    return () => {
      ...
      // Безопасное удаление canvas
      if (container && glContext.canvas.parentNode === container) {
        container.removeChild(glContext.canvas);
      }
      
      // Безопасное освобождение WebGL контекста
      try {
        glContext.getExtension("WEBGL_lose_context")?.loseContext();
      } catch (e) {
        console.warn('Не удалось освободить WebGL контекст:', e);
      }
    };
